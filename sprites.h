...
typedef struct {
  int x;
  int y;
  int timer;           // Counts down until explosion (e.g., 60 frames)
  int explosionTimer;  // Duration the explosion is visible (e.g., 15 frames)
  int active;          // 0 = inactive, 1 = active
  int oamIndex;        // For drawing the bomb sprite
} BOMB;
